...
const initDb = (db) => {
  db.serialize(() => {
    ...
    db.run(`CREATE TABLE IF NOT EXISTS pre_assessment_submissions (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      trainee_id INTEGER NOT NULL,
      competency_id INTEGER NOT NULL,
      quiz_id INTEGER NOT NULL,
      assigned_assessor_id INTEGER NOT NULL,
      reviewed_by_assessor_id INTEGER,
      submission_group_id TEXT,
      attempt_number INTEGER DEFAULT 1,
      trainee_responses TEXT NOT NULL,
      assessor_responses TEXT,
      assessor_overall_comment TEXT,
      is_passed INTEGER,
      status TEXT DEFAULT 'Assessor_Pending',
      created_at TEXT DEFAULT CURRENT_TIMESTAMP,
      updated_at TEXT
    )`, () => {
      db.all("PRAGMA table_info(pre_assessment_submissions)", (err, cols) => {
        if (cols && cols.length > 0) {
          if (!cols.some(c => c.name === 'assessor_responses')) db.run("ALTER TABLE pre_assessment_submissions ADD COLUMN assessor_responses TEXT");
          if (!cols.some(c => c.name === 'assessor_overall_comment')) db.run("ALTER TABLE pre_assessment_submissions ADD COLUMN assessor_overall_comment TEXT");
          if (!cols.some(c => c.name === 'is_passed')) db.run("ALTER TABLE pre_assessment_submissions ADD COLUMN is_passed INTEGER");
          if (!cols.some(c => c.name === 'status')) db.run("ALTER TABLE pre_assessment_submissions ADD COLUMN status TEXT DEFAULT 'Assessor_Pending'");
          if (!cols.some(c => c.name === 'reviewed_by_assessor_id')) db.run("ALTER TABLE pre_assessment_submissions ADD COLUMN reviewed_by_assessor_id INTEGER");
          if (!cols.some(c => c.name === 'submission_group_id')) db.run("ALTER TABLE pre_assessment_submissions ADD COLUMN submission_group_id TEXT");
          if (!cols.some(c => c.name === 'attempt_number')) db.run("ALTER TABLE pre_assessment_submissions ADD COLUMN attempt_number INTEGER DEFAULT 1");
          if (!cols.some(c => c.name === 'updated_at')) db.run("ALTER TABLE pre_assessment_submissions ADD COLUMN updated_at TEXT");
        }
      });
    });
    ...
    db.all("PRAGMA table_info(quizzes)", (err, cols) => {
      const hasIsViva = cols && cols.some(c => c.name === 'is_viva');
      if (!hasIsViva) db.run("ALTER TABLE quizzes ADD COLUMN is_viva INTEGER DEFAULT 0", () => {});
      const hasIsPreAssessment = cols && cols.some(c => c.name === 'is_pre_assessment');
      if (!hasIsPreAssessment) db.run("ALTER TABLE quizzes ADD COLUMN is_pre_assessment INTEGER DEFAULT 0", () => {});
    });
...
// Status rank, lowest to highest: Training < Meets Requirements < Assessment
// Pending < Competent < Competent to Train. Used to derive an overall
// "lowest common status" across a group of competencies for a user.
const STATUS_RANK = { t: 0, m: 1, a: 2, c: 3, x: 4 };

// Default status for a user/competency pair with no progress row yet,
// mirroring the auto-promotion logic applied elsewhere (a competency with
// no outstanding prerequisites starts at 'm' instead of 't').
const computeDefaultStatus = (competency, quizzesByComp) => {
  if (competency.requires_instructions) {
    try { if (JSON.parse(competency.reading_prerequisites || '[]').length > 0) return 't'; } catch(e) {}
  }
  if (competency.requires_quiz && quizzesByComp[competency.id] && quizzesByComp[competency.id].length > 0) return 't';
  if (competency.requires_prerequisite_competencies) {
    try { if (JSON.parse(competency.prerequisite_competencies || '[]').length > 0) return 't'; } catch(e) {}
  }
  if (competency.requires_pre_eval) return 't';
  let reqs = [];
  try { reqs = JSON.parse(competency.qatrack_requirements || '[]'); } catch(e) {}
  if (reqs.length === 0 && competency.required_qatrack_count > 0 && competency.qatrack_test_identifier) {
    reqs = [{ count: competency.required_qatrack_count, identifier: competency.qatrack_test_identifier }];
  }
  if (reqs.length > 0) return 't';
  if (competency.required_plan_count > 0) return 't';
  return 'm';
};

// Reduce a list of status codes to the lowest-ranked one (the "weakest
// link" status used for category/section leaderboard overviews).
const lowestCommonStatus = (statuses) => {
  if (!statuses || statuses.length === 0) return null;
  return statuses.reduce((lowest, s) => {
    if (STATUS_RANK[s] === undefined) return lowest;
    if (lowest === null || STATUS_RANK[s] < STATUS_RANK[lowest]) return s;
    return lowest;
  }, null);
};
...
const syncCompetencyInternal = async (db, user_id, username, competency_id) => {
    ...
    if (!username) {
      const uRes = await query(sharedDb, 'SELECT username FROM users WHERE id = ?', [user_id]);
      if (uRes.length > 0) username = uRes[0].username;
    }
    ...
    if (comp.requires_instructions) {
      const completedSet = new Set(rc.map(String));
      for (let r of rp) {
        if (!completedSet.has(String(r.id))) { requirementsMet = false; missingReason = `Reading prerequisite '${r.name}' not completed.`; break; }
      }
    }
    
    if (requirementsMet && comp.requires_quiz) {
        const qzs = await query(db, `SELECT q.id, q.name, q.is_viva, q.is_pre_assessment FROM quizzes q JOIN competency_quizzes cq ON q.id = cq.quiz_id WHERE cq.competency_id = ?`, [competency_id]);
        for (let qz of qzs) {
          if (qz.is_pre_assessment) {
            const passedPreAssessments = await query(db, `SELECT id FROM pre_assessment_submissions WHERE trainee_id = ? AND competency_id = ? AND quiz_id = ? AND status = 'Completed' AND is_passed = 1`, [user_id, competency_id, qz.id]);
            if (passedPreAssessments.length === 0) { requirementsMet = false; missingReason = `Pre-assessment '${qz.name}' not passed.`; break; }
          } else if (qz.is_viva) {
            ...
            const qRecord = qc[qz.id] || qc[String(qz.id)];
            if (!qRecord || !qRecord.passed) {
              requirementsMet = false; missingReason = `Quiz '${qz.name}' not passed.`; break;
            }
          }
        }
    }
    ...
    if (progressQuery.length === 0) { 
      ...
      if (currentStatus === 't' && requirementsMet) { 
        await execute(db, `UPDATE staff_competency_progress SET current_status = 'm' WHERE user_id = ? AND competency_id = ?`, [user_id, competency_id]); 
        await execute(db, `INSERT INTO competency_audit_log (target_user_id, competency_id, action_type, actioned_by_id, previous_status, new_status, notes) VALUES (?, ?, 'PROMOTED_TO_M', ?, 't', 'm', 'System auto-promotion (Prerequisites met)')`, [user_id, competency_id, user_id]); 
        return { success: true, promoted: true, status: 'm' }; 
      } else if (currentStatus === 'm' && !requirementsMet) { 
        await execute(db, `UPDATE staff_competency_progress SET current_status = 't' WHERE user_id = ? AND competency_id = ?`, [user_id, competency_id]); 
        await execute(db, `INSERT INTO competency_audit_log (target_user_id, competency_id, action_type, actioned_by_id, previous_status, new_status, notes) VALUES (?, ?, 'DEMOTED_TO_T', ?, ?, 't', 'System auto-demotion (Prerequisites no longer met)')`, [user_id, competency_id, user_id, currentStatus]); 
        return { success: true, demoted: true, reason: missingReason, status: 't' }; 
      }
    }
...
app.post('/api/switch-db', (req, res) => {
  ...
  jwt.verify(token, SECRET_KEY, async (err, userPayload) => {
    ...
    try {
      ...
      let activeIn = [];
      try { activeIn = JSON.parse(user.active_in || '[]'); } catch(e) {}
      if (!user.is_admin && !user.is_superuser && activeIn.length > 0 && !activeIn.includes(newDbName)) {
...
const authenticateToken = (req, res, next) => {
  ...
  jwt.verify(token, SECRET_KEY, (err, user) => {
    ...
    const activeIn = Array.isArray(user.active_in) ? user.active_in : [];
    if (!user.is_admin && !user.is_superuser && activeIn.length > 0 && !activeIn.includes(dbName)) {
      return res.status(403).json({ error: `Not authorized for the '${dbName}' section.` });
    }
...
const requireAdmin = (req, res, next) => {
  if (req.user && (req.user.is_admin || req.user.is_superuser)) {
    next();
  } else {
...
async function isEligibleToViewUserCompetency(db, reqUser, targetUserId, competencyId) {
  if (reqUser.is_admin || reqUser.is_superuser) return true;
  if (parseInt(reqUser.id, 10) === parseInt(targetUserId, 10)) return true;
...
app.get('/api/quizzes/library', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    const preAssessmentSubmissions = await query(req.db, `SELECT id, quiz_id, trainee_id, competency_id, submission_group_id, attempt_number FROM pre_assessment_submissions`);
    const quizStatsMap = {};
    vivas.forEach(v => {
    ...
    const groupedPreAssessmentAttempts = {};
    preAssessmentSubmissions.forEach(submission => {
      const groupKey = submission.submission_group_id ? `${submission.quiz_id}:${submission.submission_group_id}` : `${submission.quiz_id}:row:${submission.id}`;
      if (!groupedPreAssessmentAttempts[groupKey]) {
        groupedPreAssessmentAttempts[groupKey] = {
    ...
    Object.values(groupedPreAssessmentAttempts).forEach(attempt => {
      if (!quizStatsMap[attempt.quiz_id]) quizStatsMap[attempt.quiz_id] = { sumDelta: 0, sumTrainee: 0, sumAssessor: 0, count: 0, evals: 0, submissions: 0, resubmissions: 0 };
      quizStatsMap[attempt.quiz_id].submissions += 1;
      if ((attempt.attempt_number || 1) > 1) quizStatsMap[attempt.quiz_id].resubmissions += 1;
    });
    ...
    quizzes.forEach(q => {
      ...
      if (q.is_pre_assessment) {
        q.submissions_count = quizStatsMap[q.id]?.submissions || 0;
        q.resubmissions_count = quizStatsMap[q.id]?.resubmissions || 0;
        q.evals_count = q.submissions_count;
      }
    });
    res.json(quizzes);
  } catch (error) {
...
app.post('/api/quizzes/library', authenticateToken, requireAdmin, async (req, res) => {
    const { name, passing_score_percent, questions, is_viva, is_pre_assessment } = req.body;
    try {
        const result = await execute(req.db, `INSERT INTO quizzes (name, passing_score_percent, is_viva, is_pre_assessment) VALUES (?, ?, ?, ?)`, [name, passing_score_percent, is_viva ? 1 : 0, is_pre_assessment ? 1 : 0]);
...
app.put('/api/quizzes/library/:id', authenticateToken, requireAdmin, async (req, res) => {
    ...
    try {
        await execute(req.db, `UPDATE quizzes SET name = ?, passing_score_percent = ?, is_viva = ?, is_pre_assessment = ? WHERE id = ?`, [name, passing_score_percent, is_viva ? 1 : 0, is_pre_assessment ? 1 : 0, quiz_id]);
...
app.delete('/api/quizzes/library/:id', authenticateToken, requireAdmin, async (req, res) => {
    ...
    try {
        ...
        await execute(req.db, `DELETE FROM pre_assessment_submissions WHERE quiz_id = ?`, [quiz_id]);
        res.json({ success: true });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
...
app.get('/api/competency/:id/quiz', authenticateToken, async (req, res) => {
  try {
    ...
    if (!(await isEligibleToViewUserCompetency(req.db, req.user, target_user_id, req.params.id))) {
      return res.status(403).json({ error: 'Unauthorized to view quiz details for this competency' });
    }
    const progressRec = await query(req.db, `SELECT quizzes_completed FROM staff_competency_progress WHERE user_id = ? AND competency_id = ?`, [target_user_id, req.params.id]);
    ...
    const isStaffOrAdmin = req.user.is_admin || req.user.is_superuser;
    const result = [];
    for (let qz of qzs) {
      ...
      const processedQuestions = questions.map(q => {
        ...
        // Always include correct_option if the user is an admin/assessor, or if viewing another user's cell, or if the target user has taken the quiz
        if (hasTaken || isStaffOrAdmin || req.user.id !== target_user_id) {
          item.correct_option = q.correct_option;
        }
        return item;
      });
      result.push({ ...qz, questions: processedQuestions });
    }
    res.json(result);
  } catch (error) {
    res.status(500).json({ error: error.message });
...
app.post('/api/competency/:id/submit-quiz', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    await syncCompetencyInternal(req.db, user_id, req.user.username, parseInt(req.params.id, 10));
...
app.post('/api/pre-assessment/submit', authenticateToken, async (req, res) => {
  const { competency_id, quiz_id, assigned_assessor_id, trainee_responses, submission_group_id } = req.body;
  const trainee_id = req.user.id;
  try {
    const assessorId = parseInt(assigned_assessor_id, 10) || 0;
    if (assessorId !== 0) {
      const check = await query(req.db, `SELECT current_status FROM staff_competency_progress WHERE user_id = ? AND competency_id = ?`, [assessorId, competency_id]);
      if (check.length === 0 || !['x', 'x+'].includes(check[0].current_status)) return res.status(400).json({ error: 'Selected assessor is not eligible.' });
    }

    const groupId = submission_group_id || `pa-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`;
    const existingGroup = await query(req.db, `SELECT attempt_number FROM pre_assessment_submissions WHERE trainee_id = ? AND competency_id = ? AND quiz_id = ? AND submission_group_id = ? LIMIT 1`, [trainee_id, competency_id, quiz_id, groupId]);
    let attemptNumber = 1;
    if (existingGroup.length > 0) {
      attemptNumber = existingGroup[0].attempt_number || 1;
    } else {
      const attemptRow = await query(req.db, `SELECT COALESCE(MAX(attempt_number), 0) as max_attempt FROM pre_assessment_submissions WHERE trainee_id = ? AND competency_id = ? AND quiz_id = ?`, [trainee_id, competency_id, quiz_id]);
      attemptNumber = (attemptRow[0]?.max_attempt || 0) + 1;
    }

    await execute(req.db, `INSERT INTO pre_assessment_submissions (trainee_id, competency_id, quiz_id, assigned_assessor_id, submission_group_id, attempt_number, trainee_responses) VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [trainee_id, competency_id, quiz_id, assessorId, groupId, attemptNumber, JSON.stringify(trainee_responses || {})]);

    await execute(req.db, `INSERT INTO competency_audit_log (target_user_id, competency_id, action_type, actioned_by_id, notes) VALUES (?, ?, 'PRE_ASSESSMENT_SUBMITTED', ?, ?)`,
      [trainee_id, competency_id, trainee_id, assessorId === 0 ? `Submitted pre-assessment for quiz ID ${quiz_id} to all available assessors (attempt ${attemptNumber}, group ${groupId})` : `Submitted pre-assessment for quiz ID ${quiz_id} to assessor ID ${assessorId} (attempt ${attemptNumber}, group ${groupId})`]);
...
app.put('/api/pre-assessment/review/:id', authenticateToken, async (req, res) => {
  const { assessor_responses, is_passed, assessor_overall_comment } = req.body;
  const submissionId = req.params.id;
  try {
    const submission = await query(req.db, `SELECT * FROM pre_assessment_submissions WHERE id = ?`, [submissionId]);
    if (submission.length === 0) return res.status(404).json({ error: 'Pre-assessment not found' });
    if (submission[0].assigned_assessor_id !== 0 && submission[0].assigned_assessor_id !== req.user.id && !req.user.is_superuser) return res.status(403).json({ error: 'Unauthorized' });

    if ((submission[0].assigned_assessor_id === 0 || submission[0].assigned_assessor_id === null) && !req.user.is_superuser) {
      const claimResult = await execute(req.db, `UPDATE pre_assessment_submissions SET assigned_assessor_id = ? WHERE id = ? AND status = 'Assessor_Pending' AND (assigned_assessor_id = 0 OR assigned_assessor_id IS NULL)`, [req.user.id, submissionId]);
      if (!claimResult || claimResult.changes === 0) return res.status(409).json({ error: 'This submission has already been claimed by another assessor.' });
      submission[0].assigned_assessor_id = req.user.id;
    }

    const status = is_passed === 1 ? 'Completed' : 'Needs_Retake';
    await execute(req.db, `UPDATE pre_assessment_submissions SET assessor_responses = ?, assessor_overall_comment = ?, is_passed = ?, status = ?, reviewed_by_assessor_id = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`,
      [JSON.stringify(assessor_responses || {}), assessor_overall_comment || null, is_passed, status, req.user.id, submissionId]);

    await execute(req.db, `INSERT INTO competency_audit_log (target_user_id, competency_id, action_type, actioned_by_id, notes) VALUES (?, ?, 'PRE_ASSESSMENT_REVIEWED', ?, ?)`,
      [submission[0].trainee_id, submission[0].competency_id, req.user.id, `Reviewed pre-assessment ID ${submissionId}. Status: ${status}`]);

    const trainee = await query(sharedDb, `SELECT username FROM users WHERE id = ?`, [submission[0].trainee_id]);
    if (trainee.length > 0) {
      await syncCompetencyInternal(req.db, submission[0].trainee_id, trainee[0].username, submission[0].competency_id);
    }
...
app.get('/api/pre-assessment/summary/:user_id/:competency_id', authenticateToken, async (req, res) => {
  const { user_id, competency_id } = req.params;
  try {
    if (!(await isEligibleToViewUserCompetency(req.db, req.user, user_id, competency_id))) {
      return res.status(403).json({ error: 'Unauthorized to view pre-assessment summary for this competency' });
    }
    const submissions = await query(req.db, `SELECT p.*, q.name as quiz_name FROM pre_assessment_submissions p JOIN quizzes q ON p.quiz_id = q.id WHERE p.trainee_id = ? AND p.competency_id = ? ORDER BY p.created_at DESC`, [user_id, competency_id]);
    const users = await query(sharedDb, `SELECT id, full_name FROM users`);
    const userMap = {}; users.forEach(u => userMap[u.id] = u.full_name);
    for (let sub of submissions) {
      sub.questions = await query(req.db, `SELECT id, question_text FROM quiz_questions WHERE quiz_id = ?`, [sub.quiz_id]);
      const assessedById = sub.reviewed_by_assessor_id || sub.assigned_assessor_id;
      sub.assessor_name = userMap[assessedById] || (sub.status === 'Assessor_Pending' && sub.assigned_assessor_id === 0 ? 'Any eligible assessor' : 'Unknown');
      try { sub.trainee_responses = JSON.parse(sub.trainee_responses || '{}'); } catch (e) { sub.trainee_responses = {}; }
      try { sub.assessor_responses = JSON.parse(sub.assessor_responses || '{}'); } catch (e) { sub.assessor_responses = {}; }
    }
    res.json(submissions);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
app.get('/api/competency/:id/qatrack-evidence', authenticateToken, async (req, res) => {
  ...
  try {
    if (!(await isEligibleToViewUserCompetency(req.db, req.user, target_user_id, competency_id))) {
      return res.status(403).json({ error: 'Unauthorized to view QA evidence for this competency' });
    }
...
app.get('/api/competency/:id/evaluations', authenticateToken, async (req, res) => {
  ...
  try {
    if (!(await isEligibleToViewUserCompetency(req.db, req.user, user_id, competency_id))) {
      return res.status(403).json({ error: 'Unauthorized to view evaluations for this competency' });
    }
...
app.get('/api/competency/:id/files', authenticateToken, async (req, res) => {
  ...
  try {
    if (!(await isEligibleToViewUserCompetency(req.db, req.user, target_user_id, competency_id))) {
      return res.status(403).json({ error: 'Unauthorized to view files for this competency' });
    }
...
app.put('/api/planning-logs/:id', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (!['Draft', 'Pending_Review', 'Needs_Amendment'].includes(log[0].status)) return res.status(400).json({ error: "Cannot edit an assessed log." });
    const status = is_draft ? 'Draft' : 'Pending_Review';
    let assessor_id = assigned_assessor_id || 0;
    if (!is_draft && !assessor_id) return res.status(400).json({ error: "Missing assessor" });
    if (!is_draft) {
...
app.get('/api/competency/:id/planning-logs', authenticateToken, async (req, res) => {
  ...
  try {
    if (!(await isEligibleToViewUserCompetency(req.db, req.user, target_user_id, req.params.id))) {
      return res.status(403).json({ error: 'Unauthorized to view case logs for this competency' });
    }
    const logs = await query(req.db, `SELECT p.*, c.task_name, c.category FROM patient_plan_logs p JOIN competencies c ON p.competency_id = c.id WHERE p.trainee_id = ? AND p.competency_id = ? ORDER BY p.created_at DESC`, [target_user_id, req.params.id]);
    const users = await query(sharedDb, `SELECT id, full_name FROM users`);
    const userMap = {}; users.forEach(u => userMap[u.id] = u.full_name);
...
function updateAssessorCommentsTrail(existingText, trainerName, newContent, isDraft, score) {
  ...
  if (existingText) {
    while ((match = regex.exec(existingText)) !== null) {
      if (lastHeader) {
        const content = existingText.slice(lastIndex, match.index).trim();
        entries.push({ trainer: lastTrainer, date: lastDate, header: lastHeader, content });
      }
      lastHeader = match[0].trim();
      lastTrainer = match[1].trim();
      lastDate = match[2].trim();
      lastIndex = regex.lastIndex;
    }
    if (lastHeader) {
      const content = existingText.slice(lastIndex).trim();
      entries.push({ trainer: lastTrainer, date: lastDate, header: lastHeader, content });
    }
  }
  ...
  let myHeader = '';
  if (isDraft) {
    myHeader = `[Saved by ${trainerName} on ${dateStr}, ${timeStr}]:`;
  } else {
...
app.put('/api/planning-logs/review/:id', authenticateToken, async (req, res) => {
  const { score, assessor_comments, is_draft } = req.body;
  const logId = req.params.id;
  try {
    const log = await query(req.db, `SELECT * FROM patient_plan_logs WHERE id = ?`, [logId]);
    if (log.length === 0) return res.status(404).json({ error: "Log not found" });

    const trainerCheck = await query(req.db, `SELECT current_status FROM staff_competency_progress WHERE user_id = ? AND competency_id = ?`, [req.user.id, log[0].competency_id]);
    const isEligibleTrainer = trainerCheck.length > 0 && ['x', 'x+'].includes(trainerCheck[0].current_status);
    if (log[0].assigned_assessor_id !== req.user.id && !req.user.is_superuser && !isEligibleTrainer) {
      return res.status(403).json({ error: "Unauthorized" });
    }

    const trainer = await query(sharedDb, `SELECT full_name, username FROM users WHERE id = ?`, [req.user.id]);
    const trainerName = (trainer.length > 0 && trainer[0].full_name) ? trainer[0].full_name : req.user.username;

    if (is_draft) {
      const finalComments = updateAssessorCommentsTrail(log[0].assessor_comments, trainerName, assessor_comments, true, null);

      await execute(req.db, `UPDATE patient_plan_logs SET assessor_comments = ?, assigned_assessor_id = ? WHERE id = ?`, [finalComments || null, req.user.id, logId]);
      await execute(req.db, `INSERT INTO competency_audit_log (target_user_id, competency_id, action_type, actioned_by_id, notes) VALUES (?, ?, 'CASE_LOG_SAVED', ?, ?)`, [log[0].trainee_id, log[0].competency_id, req.user.id, `Saved feedback for case log ${log[0].patient_reference}`]);
      ...
      await execute(req.db, `UPDATE patient_plan_logs SET score = ?, assessor_comments = ?, status = ?, assigned_assessor_id = ?, reviewed_at = CURRENT_TIMESTAMP WHERE id = ?`, [score, finalComments || null, status, req.user.id, logId]);
      await execute(req.db, `INSERT INTO competency_audit_log (target_user_id, competency_id, action_type, actioned_by_id, notes) VALUES (?, ?, 'CASE_LOG_COMPLETED', ?, ?)`, [log[0].trainee_id, log[0].competency_id, req.user.id, `Completed case log ${log[0].patient_reference}. Supervision: ${levelText}. Status: ${status}`]);
      ...
      return res.json({ success: true, status, assessor_comments: finalComments });
    }
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.get('/api/viva/summary/:user_id/:competency_id', authenticateToken, async (req, res) => {
  ...
  try {
    if (!(await isEligibleToViewUserCompetency(req.db, req.user, user_id, competency_id))) {
      return res.status(403).json({ error: 'Unauthorized to view viva evaluations for this competency' });
    }
...
app.post('/api/competency/milestone', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const syncResult = await syncCompetencyInternal(req.db, user_id, req.user.username, competency_id);
...
app.post('/api/competency/sync', authenticateToken, async (req, res) => {
  ...
  let username = req.user.username; // Derived from JWT payload

  if (req.body.user_id && req.body.user_id !== req.user.id) {
    // Only administrators/assessors can sync other users
    if (req.user.is_admin || req.user.is_superuser) {
      user_id = parseInt(req.body.user_id, 10);
      const userQuery = await query(sharedDb, 'SELECT username FROM users WHERE id = ?', [user_id]);
      if (userQuery.length === 0) return res.status(404).json({ error: 'User not found' });
      username = userQuery[0].username;
    } else {
      return res.status(403).json({ error: 'Forbidden: Only assessors can sync other users.' });
    }
  }

  try {
    const result = await syncCompetencyInternal(req.db, user_id, username, competency_id);
...
app.post('/api/competency/revert-assessment', authenticateToken, async (req, res) => {
  ...
  try {
    const progressCheck = await query(req.db, `SELECT current_status FROM staff_competency_progress WHERE user_id = ? AND competency_id = ?`, [user_id, competency_id]);
    const curStatus = progressCheck.length > 0 ? (progressCheck[0].current_status || '').toLowerCase() : '';
    if (curStatus === 'a') {
      ...
      await execute(req.db, `INSERT INTO competency_audit_log (target_user_id, competency_id, action_type, actioned_by_id, previous_status, new_status, notes) VALUES (?, ?, 'REVERTED_TO_M', ?, 'a', 'm', 'User manually reverted assessment request')`, [user_id, competency_id, req.user.id]);
      res.json({ success: true });
    } else {
...
app.post('/api/progress/admin-force-status', authenticateToken, async (req, res) => {
  const user_id = parseInt(req.body.user_id, 10);
  const competency_id = parseInt(req.body.competency_id, 10);
  const { status, signoff_comment, date_override } = req.body;
  ...
  try {
    ...
    const isAssessor = !req.user.is_admin && !req.user.is_superuser;
    const actionType = isAssessor ? 'ASSESSOR_FORCE_STATUS' : 'ADMIN_FORCE_STATUS';
    const auditNotes = isAssessor ? `Assessor forced status change to '${status}'` : `Admin forced status change to '${status}'`;

    await execute(req.db, 
      `INSERT INTO competency_audit_log (target_user_id, competency_id, action_type, actioned_by_id, previous_status, new_status, notes) VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [user_id, competency_id, actionType, admin_id, previousStatus, status, auditNotes]
    );
...
app.post('/api/progress/admin-update', authenticateToken, async (req, res) => {
  const user_id = parseInt(req.body.user_id, 10);
  const competency_id = parseInt(req.body.competency_id, 10);
  const { readings_completed } = req.body;
  const admin_id = req.user.id;
  ...
  try {
    ...
    const isAssessor = !req.user.is_admin && !req.user.is_superuser;
    const actionType = isAssessor ? 'ASSESSOR_OVERRIDE' : 'ADMIN_OVERRIDE';
    const auditNotes = isAssessor ? 'Assessor updated prerequisites manually' : 'Admin updated prerequisites manually';

    await execute(req.db, 
      `INSERT INTO competency_audit_log (target_user_id, competency_id, action_type, actioned_by_id, notes) VALUES (?, ?, ?, ?, ?)`,
      [user_id, competency_id, actionType, admin_id, auditNotes]
    );
    await syncCompetencyInternal(req.db, user_id, null, competency_id);
...
app.post('/api/progress/admin-reset-quiz', authenticateToken, async (req, res) => {
  const user_id = parseInt(req.body.user_id, 10);
  const competency_id = parseInt(req.body.competency_id, 10);
  ...
  try {
    ...
    if (quiz_id) {
      ...
      await execute(req.db, `DELETE FROM pre_assessment_submissions WHERE trainee_id = ? AND competency_id = ? AND quiz_id = ?`, [user_id, competency_id, quiz_id]);
    } else {
      await execute(req.db, `DELETE FROM viva_evaluations WHERE trainee_id = ? AND competency_id = ?`, [user_id, competency_id]);
      await execute(req.db, `DELETE FROM pre_assessment_submissions WHERE trainee_id = ? AND competency_id = ?`, [user_id, competency_id]);
    }
    ...
    const isAssessor = !req.user.is_admin && !req.user.is_superuser;
    const actionType = isAssessor ? 'ASSESSOR_QUIZ_RESET' : 'ADMIN_QUIZ_RESET';
    const auditNotes = isAssessor ? 'Assessor reset quiz progress' : 'Admin reset quiz progress';
...
app.post('/api/progress/admin-pass-quiz', authenticateToken, async (req, res) => {
  const user_id = parseInt(req.body.user_id, 10);
  const competency_id = parseInt(req.body.competency_id, 10);
  const { quiz_id } = req.body;
  const admin_id = req.user.id;
  try {
    ...
    const isAssessor = !req.user.is_admin && !req.user.is_superuser;
    const actionType = isAssessor ? 'ASSESSOR_QUIZ_PASS' : 'ADMIN_QUIZ_PASS';
    const auditNotes = isAssessor ? 'Assessor forced quiz as passed' : 'Admin forced quiz as passed';

    await execute(req.db, `INSERT INTO competency_audit_log (target_user_id, competency_id, action_type, actioned_by_id, notes) VALUES (?, ?, ?, ?, ?)`, [user_id, competency_id, actionType, admin_id, auditNotes]);
    await syncCompetencyInternal(req.db, user_id, null, competency_id);
    res.json({ success: true, message: "Quiz marked as passed." });
  } catch (err) { res.status(500).json({ error: err.message }); }
});

app.post('/api/progress/admin-pass-eval', authenticateToken, async (req, res) => {
  const user_id = parseInt(req.body.user_id, 10);
  const competency_id = parseInt(req.body.competency_id, 10);
  const { eval_type } = req.body;
  const admin_id = req.user.id;
  try {
    if (!(await isEligibleAssessorForUserCompetency(req.db, req.user, user_id, competency_id))) {
      return res.status(403).json({ error: "Access Denied: Requires Administrator privileges or Assessor 'X' status for this competency." });
    ...
    const isAssessor = !req.user.is_admin && !req.user.is_superuser;
    const actionType = isAssessor ? 'ASSESSOR_EVAL_PASS' : 'ADMIN_EVAL_PASS';
    const auditNotes = isAssessor ? `Assessor forced ${eval_type}-training evaluation as passed` : `Admin forced ${eval_type}-training evaluation as passed`;
...
app.post('/api/progress/admin-reset-eval', authenticateToken, async (req, res) => {
  const user_id = parseInt(req.body.user_id, 10);
  const competency_id = parseInt(req.body.competency_id, 10);
  const { eval_type } = req.body;
  const admin_id = req.user.id;
  try {
    ...
    const isAssessor = !req.user.is_admin && !req.user.is_superuser;
    const actionType = isAssessor ? 'ASSESSOR_EVAL_RESET' : 'ADMIN_EVAL_RESET';
    const auditNotes = isAssessor ? `Assessor reset ${eval_type}-training evaluation` : `Admin reset ${eval_type}-training evaluation`;
...
// --- STATISTICS ---
app.get('/api/admin/competency/:id/records-summary', authenticateToken, requireAdmin, async (req, res) => {
  const competencyId = parseInt(req.params.id, 10);
  try {
    const compResult = await query(req.db, 'SELECT * FROM competencies WHERE id = ?', [competencyId]);
    if (compResult.length === 0) return res.status(404).json({ error: 'Competency not found' });
    const competency = compResult[0];

    // Parse requirements
    let qatrackReqs = [];
    try { qatrackReqs = JSON.parse(competency.qatrack_requirements || '[]'); } catch(e) {}
    if (qatrackReqs.length === 0 && competency.required_qatrack_count > 0 && competency.qatrack_test_identifier) {
      qatrackReqs = [{ count: competency.required_qatrack_count, identifier: competency.qatrack_test_identifier }];
    }

    // Get active users for the current database
    const dbName = req.headers['x-database'] || 'QA';
    const sections = await getSections();
    const currentSection = sections.find(s => s.name === dbName);
    const leaderboardType = currentSection?.leaderboardType || (dbName === 'QA' ? 'qa' : 'logbook');

    // Which leaderboard to show is dictated by the section's "leaderboard basis"
    // toggle, not by which data the competency happens to have.
    const hasCaseLogs = leaderboardType === 'logbook';
    const hasQATrack = leaderboardType === 'qa';

    const allUsers = await query(sharedDb, 'SELECT id, username, full_name, designation, active_in FROM users WHERE is_active = 1');
    const activeUsers = allUsers.filter(u => {
      let activeIn = [];
      try { activeIn = JSON.parse(u.active_in || '[]'); } catch(e){}
      return activeIn.includes(dbName);
    });

    // Check if competency is applicable to each user
    const competencyGroups = await query(req.db, 'SELECT group_name FROM competency_groups WHERE competency_id = ?', [competencyId]);
    const targetGroups = competencyGroups.map(cg => cg.group_name);
    let targetUsers = [];
    try { targetUsers = JSON.parse(competency.target_users || '[]'); } catch(e) {}

    const applicableUsers = activeUsers.filter(u => {
    ...
    // Build summary list
    const summary = applicableUsers.map(user => {
      const qaDetail = progressMap[user.id] || {};
      let qaTotalCount = 0;
      const qaBreakdown = qatrackReqs.map(req => {
        const count = qaDetail[req.identifier] || 0;
        qaTotalCount += count;
        return { identifier: req.identifier, count, required: req.count };
      });
    ...
    res.json({
      competency: {
        id: competency.id,
        task_name: competency.task_name,
...
app.get('/api/admin/category/records-summary', authenticateToken, requireAdmin, async (req, res) => {
  const category = req.query.category;
  if (!category) return res.status(400).json({ error: 'Category is required' });
  try {
    const competencies = await query(req.db, 'SELECT * FROM competencies WHERE category = ?', [category]);
    if (competencies.length === 0) return res.status(404).json({ error: 'No competencies found for this category' });
    ...
    // Check if any of these competencies are applicable to each user
    const competencyGroups = await query(req.db, `SELECT competency_id, group_name FROM competency_groups WHERE competency_id IN (${competencyIds.map(() => '?').join(',')})`, competencyIds);
    
    const compGroupsMap = {};
    competencyGroups.forEach(cg => {
      if (!compGroupsMap[cg.competency_id]) compGroupsMap[cg.competency_id] = [];
      compGroupsMap[cg.competency_id].push(cg.group_name);
    });

    const applicableUsers = activeUsers.filter(u => {
      return competencies.some(c => {
        const targetGroups = compGroupsMap[c.id] || [];
        let targetUsers = [];
        try { targetUsers = JSON.parse(c.target_users || '[]'); } catch(e) {}
        return targetGroups.includes(u.designation) || targetUsers.includes(u.id);
      });
    });

    // Compute each user's overall status across all applicable competencies
    // in this category: the lowest-ranked status is shown (one weak link
    // drags the whole group's displayed status down).
    const competencyQuizzes = await query(req.db, `SELECT competency_id, quiz_id FROM competency_quizzes WHERE competency_id IN (${competencyIds.map(() => '?').join(',')})`, competencyIds);
    const quizzesByComp = {};
    competencyQuizzes.forEach(cq => {
      if (!quizzesByComp[cq.competency_id]) quizzesByComp[cq.competency_id] = [];
      quizzesByComp[cq.competency_id].push(cq.quiz_id);
    ...
    const statusProgressRows = await query(req.db, `SELECT user_id, competency_id, current_status FROM staff_competency_progress WHERE competency_id IN (${competencyIds.map(() => '?').join(',')})`, competencyIds);
    const statusProgressMap = {};
    statusProgressRows.forEach(row => {
      if (!statusProgressMap[row.user_id]) statusProgressMap[row.user_id] = {};
      statusProgressMap[row.user_id][row.competency_id] = row.current_status;
    });

    const userOverallStatus = {};
    applicableUsers.forEach(u => {
      const applicableCompsForUser = competencies.filter(c => {
        const targetGroups = compGroupsMap[c.id] || [];
        let targetUsers = [];
        try { targetUsers = JSON.parse(c.target_users || '[]'); } catch(e) {}
        return targetGroups.includes(u.designation) || targetUsers.includes(u.id);
      });
      const statuses = applicableCompsForUser.map(c => {
        const existing = statusProgressMap[u.id]?.[c.id];
        return existing || computeDefaultStatus(c, quizzesByComp);
      });
      userOverallStatus[u.id] = lowestCommonStatus(statuses);
    });

    // Separate competencies into QATrack and logbook
    const sections = await getSections();
    const currentSection = sections.find(s => s.name === dbName);
    const leaderboardType = currentSection?.leaderboardType || (dbName === 'QA' ? 'qa' : 'logbook');
    const isQA = leaderboardType === 'qa';
    // Follow the section's leaderboard toggle for ALL competencies in the
    // category, rather than guessing per-competency based on which fields
    // happen to be populated.
    const caseComps = isQA ? [] : competencies;
    const qaComps = isQA ? competencies : [];
    ...
    if (caseComps.length > 0) {
      ...
      caseLogRows.forEach(row => {
        if (userRecordsMap[row.trainee_id] !== undefined) {
    ...
    if (qaComps.length > 0) {
      ...
      progressRows.forEach(row => {
        ...
        let sum = 0;
        Object.values(detail).forEach(count => {
    ...
    // Build summary list
    const summary = applicableUsers.map(user => {
      const qaTotalCount = userRecordsMap[user.id] || 0;
      return {
        userId: user.id,
        fullName: user.full_name,
        designation: user.designation,
...
app.get('/api/admin/section/records-summary', authenticateToken, requireAdmin, async (req, res) => {
  try {
    const competencies = await query(req.db, 'SELECT * FROM competencies');
    if (competencies.length === 0) return res.status(404).json({ error: 'No competencies found in this section' });
    const competencyIds = competencies.map(c => c.id);
    ...
    // Check if any competency in the section is applicable to each user
    const competencyGroups = await query(req.db, 'SELECT competency_id, group_name FROM competency_groups');
    const compGroupsMap = {};
    competencyGroups.forEach(cg => {
      if (!compGroupsMap[cg.competency_id]) compGroupsMap[cg.competency_id] = [];
      compGroupsMap[cg.competency_id].push(cg.group_name);
    ...
    // Compute each user's overall status across all applicable competencies
    // in this section: the lowest-ranked status is shown (one weak link
    // drags the whole group's displayed status down).
    const competencyQuizzes = await query(req.db, 'SELECT competency_id, quiz_id FROM competency_quizzes');
    const quizzesByComp = {};
    competencyQuizzes.forEach(cq => {
      if (!quizzesByComp[cq.competency_id]) quizzesByComp[cq.competency_id] = [];
      quizzesByComp[cq.competency_id].push(cq.quiz_id);
    ...
    const statusProgressRows = await query(req.db, 'SELECT user_id, competency_id, current_status FROM staff_competency_progress');
    const statusProgressMap = {};
    statusProgressRows.forEach(row => {
      if (!statusProgressMap[row.user_id]) statusProgressMap[row.user_id] = {};
      statusProgressMap[row.user_id][row.competency_id] = row.current_status;
    ...
    const isQA = leaderboardType === 'qa';
    // Follow the section's leaderboard toggle for ALL competencies in the
    // section, rather than guessing per-competency based on which fields
    // happen to be populated.
    const caseComps = isQA ? [] : competencies;
    const qaComps = isQA ? competencies : [];
    ...
    res.json({
      competency: {
        task_name: 'Section Leaderboard',
        category: `${dbName} Section`,
        hasQATrack,
        hasCaseLogs
      },
      summary
...
app.post('/api/admin/settings', authenticateToken, requireSuperuser, async (req, res) => {
  const { default_renewal_period, qatrack_api_url, qatrack_api_token, enforce_prerequisites_for_competent } = req.body;
  try {
    ...
    if (enforce_prerequisites_for_competent !== undefined) {
      await execute(sharedDb, "INSERT OR REPLACE INTO global_settings (key, value) VALUES ('enforce_prerequisites_for_competent', ?)", [(enforce_prerequisites_for_competent ? '1' : '0')]);
    }
    res.json({ success: true });
  } catch(e) { res.status(500).json({error: e.message}); }
});
...
async function getCompetenciesWithGroups(dbInstance) {
  const comps = await query(dbInstance, "SELECT * FROM competencies ORDER BY display_order ASC, id ASC");
  let compGroups = [];
  try {
    compGroups = await query(dbInstance, "SELECT competency_id, group_name FROM competency_groups");
  } catch(e) {}
  ...
  comps.forEach(c => {
    c.target_groups = groupsByComp[c.id] || [];
    try {
      let tu = typeof c.target_users === 'string' ? JSON.parse(c.target_users || '[]') : c.target_users;
      while (typeof tu === 'string') tu = JSON.parse(tu);
      c.target_users = Array.isArray(tu) ? tu : [];
    } catch(e) {
      c.target_users = [];
...
function isUserApplicableForComp(user, comp) {
  if (!user || !comp) return false;
  const inGroup = Array.isArray(comp.target_groups) && comp.target_groups.includes(user.designation);
  const inUsers = Array.isArray(comp.target_users) && comp.target_users.includes(user.id);
  return inGroup || inUsers;
}

app.get('/api/admin/reports/all-training-records-zip', authenticateToken, requireAdmin, async (req, res) => {
  try {
    const targetSection = req.query.section || 'all';
    let activeUsers = await query(sharedDb, "SELECT * FROM users WHERE is_active = 1 ORDER BY full_name ASC");
    ...
    if (targetSection !== 'all') {
      activeUsers = activeUsers.filter(u => {
        try {
          ...
          return Array.isArray(act) && act.includes(targetSection);
        } catch(e) { return true; }
      });
      if (activeUsers.length === 0) {
        return res.status(404).json({ error: `No active users found for section: ${targetSection}` });
      }
    }
    ...
    const sections = await getSections();
    let activeSections = sections.filter(s => s.active !== false);
    if (targetSection !== 'all') {
      activeSections = activeSections.filter(s => s.name === targetSection);
    }
    ...
    for (const sec of activeSections) {
      ...
      const comps = await getCompetenciesWithGroups(dbInstance);
    ...
    for (const u of activeUsers) {
      ...
      const pdfBuffer = await new Promise((resolve, reject) => {
        ...
        for (const sec of activeSections) {
          ...
          const userApplicableComps = sData.competencies.filter(c => isUserApplicableForComp(u, c));
          if (userApplicableComps.length === 0) continue;
          ...
          for (let i = 0; i < userApplicableComps.length; i++) {
            const comp = userApplicableComps[i];
    ...
    const zipFilename = targetSection === 'all'
      ? `all_users_training_records_${new Date().toISOString().split('T')[0]}.zip`
      : `users_training_records_${targetSection.toLowerCase()}_${new Date().toISOString().split('T')[0]}.zip`;
...
app.get('/api/admin/reports/overview-matrix-pdf/:section?', authenticateToken, requireAdmin, async (req, res) => {
  const targetSection = req.params.section || req.query.section || 'all';
  try {
    const pdfBuffer = await generateOverviewMatrixPdfBuffer(targetSection);
...
function drawFormCheckbox(doc, x, y, checked, size = 10) {
  doc.save();
  doc.rect(x, y, size, size).lineWidth(0.8).strokeColor('#475569').stroke();
  if (checked) {
    doc.rect(x + 0.5, y + 0.5, size - 1, size - 1).fillColor('#dcfce7').fill();
    doc.save();
    doc.lineWidth(1.6)
       .strokeColor('#15803d')
       .lineCap('round')
       .lineJoin('round')
       .moveTo(x + 2, y + size * 0.52)
       .lineTo(x + size * 0.4, y + size - 2.5)
...
function renderCompetencyAssessmentForm(doc, data) {
  const { section, comp, allCompsMap, quizzesMap, user, progress, userAllProgressMap, userMap } = data;
  const isBlank = !user;

  // Header Banner
  doc.rect(36, 36, 523, 38).fill('#1e3a8a');
  doc.fillColor('#ffffff').fontSize(12).font('Helvetica-Bold').text('IR(ME)R Clinical Competency & Assessment Record', 46, 43, { lineBreak: false });
  doc.fontSize(8).font('Helvetica').text(`Section: ${section} | Category: ${comp.category || 'General'}`, 46, 58, { lineBreak: false });
  doc.fontSize(8).font('Helvetica-Bold').text(`Renewal: ${comp.renewal_period_months || 36} Months`, 440, 58, { lineBreak: false, align: 'right', width: 110 });

  // Task Name Box
  doc.rect(36, 80, 523, 32).fillAndStroke('#f8fafc', '#cbd5e1');
  doc.fillColor('#0f172a').fontSize(8).font('Helvetica-Bold').text('COMPETENCY TASK / OBJECTIVE:', 44, 86, { lineBreak: false });
  doc.fontSize(10).font('Helvetica-Bold').fillColor('#1e3a8a').text(comp.task_name || 'Unnamed Competency', 44, 97, { width: 505, lineBreak: false, ellipsis: true });

  // Trainee Metadata Box
  doc.rect(36, 118, 523, 44).fillAndStroke('#ffffff', '#cbd5e1');
  doc.fillColor('#334155').fontSize(7.5).font('Helvetica-Bold');
  
  doc.text('Candidate Name:', 44, 125, { lineBreak: false });
  doc.font('Helvetica').text(isBlank ? '____________________________________' : (user.full_name || 'N/A'), 125, 125, { width: 180, lineBreak: false });

  doc.font('Helvetica-Bold').text('Staff ID / Username:', 320, 125, { lineBreak: false });
  doc.font('Helvetica').text(isBlank ? '________________________' : (user.username || 'N/A'), 415, 125, { width: 135, lineBreak: false });

  doc.font('Helvetica-Bold').text('Designation:', 44, 142, { lineBreak: false });
  doc.font('Helvetica').text(isBlank ? '____________________________________' : (user.designation || 'N/A'), 125, 142, { width: 180, lineBreak: false });

  doc.font('Helvetica-Bold').text('Date Started:', 320, 142, { lineBreak: false });
  doc.font('Helvetica').text(isBlank ? '____ / ____ / ________' : (progress && progress.date_started ? new Date(progress.date_started).toLocaleDateString('en-GB') : 'Not Recorded'), 415, 142, { width: 135, lineBreak: false });

  // Section Header: Prerequisites & Milestones
  let currentY = 172;
  doc.rect(36, currentY, 523, 18).fill('#0f172a');
  doc.fillColor('#ffffff').fontSize(8.5).font('Helvetica-Bold').text('PREREQUISITE TASKS & TRAINING MILESTONES CHECKLIST', 44, currentY + 5, { lineBreak: false });
  currentY += 22;

  // Table Header for prerequisites
  doc.rect(36, currentY, 523, 16).fill('#f1f5f9');
  doc.fillColor('#475569').fontSize(7).font('Helvetica-Bold');
  doc.text('Done', 42, currentY + 4, { width: 25, align: 'center', lineBreak: false });
  doc.text('Type', 72, currentY + 4, { width: 65, lineBreak: false });
  doc.text('Prerequisite Requirement / Item Description', 142, currentY + 4, { width: 230, lineBreak: false });
  doc.text('Verification / Evidence', 380, currentY + 4, { width: 170, lineBreak: false });
  currentY += 18;

  // Collect all prerequisites
  const items = [];

  // 1. Reading prerequisites
  if (comp.requires_instructions) {
    let rps = [];
    try { rps = JSON.parse(comp.reading_prerequisites || '[]'); } catch(e) {}
    if (rps.length === 0) rps = [{ id: 'default', name: 'Read Protocol Instructions & Standard Operating Procedures' }];
    rps.forEach(rp => {
      let isDone = false;
      if (!isBlank && progress) {
        try {
          const doneList = JSON.parse(progress.readings_completed || '[]');
          if (Array.isArray(doneList) && doneList.includes(rp.id)) isDone = true;
          else if (progress.instructions_read === 1) isDone = true;
        } catch(e) {}
      }
      items.push({
        type: 'Reading',
        name: rp.name || 'Protocol Instruction',
        done: isDone,
        evidence: isBlank ? 'Date: _________  Sign: _________' : (isDone ? 'Completed' : 'Pending')
      });
    });
  }

  // 2. Prerequisite Competencies
  if (comp.requires_prerequisite_competencies) {
    let pComps = [];
    try { pComps = JSON.parse(comp.prerequisite_competencies || '[]'); } catch(e) {}
    pComps.forEach(pcId => {
      const pc = (allCompsMap && allCompsMap[pcId]) ? allCompsMap[pcId] : { task_name: `Competency #${pcId}` };
      let isDone = false;
      if (!isBlank && userAllProgressMap) {
        const pcProg = userAllProgressMap[pcId];
        if (pcProg && ['c', 'x'].includes(pcProg.current_status)) isDone = true;
      }
      items.push({
        type: 'Prerequisite',
        name: pc.task_name,
        done: isDone,
        evidence: isBlank ? 'Date Achieved: _________' : (isDone ? 'Achieved (Competent)' : 'Not Yet Achieved')
      });
    });
  }
  ...
  if (comp.requires_quiz) {
    let qIds = [];
    try { qIds = JSON.parse(comp.quiz_ids || '[]'); } catch(e) {}
    if (qIds.length === 0) qIds = ['standard'];
    qIds.forEach(qId => {
      const q = (quizzesMap && quizzesMap[qId]) ? quizzesMap[qId] : { title: 'Competency Knowledge Quiz' };
      let isDone = false;
      let scoreText = isBlank ? 'Score: ____%  Date: _________' : 'Not Passed';
      if (!isBlank && progress) {
        try {
          const qDone = JSON.parse(progress.quizzes_completed || '{}');
          if (qDone[qId] && qDone[qId].passed) {
            isDone = true;
            scoreText = 'Passed (' + (qDone[qId].score !== undefined ? qDone[qId].score + '%' : '100%') + ')';
          } else if (progress.quiz_passed === 1) {
            isDone = true;
            scoreText = 'Passed' + (progress.quiz_score !== null ? ' (' + progress.quiz_score + '%)' : '');
          }
        } catch(e) {}
      }
      items.push({
        type: 'Quiz / Exam',
        name: q.title || 'Knowledge Assessment Quiz',
        done: isDone,
        evidence: scoreText
      });
    });
  ...
  // 4. QATrack / Practical Evidence
  if (comp.qatrack_requirements) {
    let qReqs = [];
    try { qReqs = JSON.parse(comp.qatrack_requirements || '[]'); } catch(e) {}
    qReqs.forEach(qr => {
      const reqCount = qr.count || 1;
      let doneCount = 0;
      if (!isBlank && progress) {
        try {
          const qd = JSON.parse(progress.qatrack_records_detail || '{}');
          doneCount = qd[qr.identifier] || progress.qatrack_records || 0;
        } catch(e) {}
      }
      const isDone = doneCount >= reqCount;
      items.push({
        type: 'Practical QC',
        name: `QC Test: ${qr.identifier || 'Machine Quality Control'} (Req: ${reqCount} reps)`,
        done: isDone,
        evidence: isBlank ? `Completed: ____ / ${reqCount}` : `${doneCount} / ${reqCount}${isDone ? ' (Completed)' : ''}`
      });
    });
  }

  // 5. Case logs / Planning logs
  if (comp.required_plan_count > 0 || comp.requires_case_logs) {
    const reqPlans = comp.required_plan_count || 1;
    items.push({
      type: 'Case Logs',
      name: `Clinical Case Logs (Required: ${reqPlans} cases)`,
      done: false,
      evidence: isBlank ? `Logged: ____ / ${reqPlans}` : 'Logged / Verified'
    });
  }
  ...
  if (items.length === 0) {
    items.push({
      type: 'Standard',
      name: 'Practical clinical training and assessment of procedure under supervision',
      done: !isBlank && progress && ['c', 'x', 'm'].includes(progress.current_status),
      evidence: isBlank ? 'Date: _________  Sign: _________' : (progress && ['c', 'x', 'm'].includes(progress.current_status) ? 'Completed' : 'In Progress')
    });
  }
  ...
  items.forEach((item, idx) => {
    ...
    drawFormCheckbox(doc, 48, rowY + 3, item.done, 9);
    doc.fillColor('#64748b').fontSize(6.5).font('Helvetica-Bold').text(item.type, 72, rowY + 4, { width: 65, lineBreak: false });
    doc.fillColor('#1e293b').fontSize(7.5).font('Helvetica').text(item.name, 142, rowY + 4, { width: 230, lineBreak: false, ellipsis: true });
    doc.fillColor('#334155').fontSize(7).font('Helvetica').text(item.evidence, 380, rowY + 4, { width: 170, lineBreak: false, ellipsis: true });

    currentY = rowY + 16;
  });

  // Assessor Sign-Off Card
  currentY += 8;
  doc.rect(36, currentY, 523, 18).fill('#0f172a');
  doc.fillColor('#ffffff').fontSize(8.5).font('Helvetica-Bold').text('CLINICAL ASSESSMENT & FORMAL SIGN-OFF', 44, currentY + 5, { lineBreak: false });
  currentY += 22;

  // Level Awarded Boxes
  doc.rect(36, currentY, 523, 26).fillAndStroke('#f8fafc', '#cbd5e1');
  doc.fillColor('#0f172a').fontSize(7.5).font('Helvetica-Bold').text('Competency Level Awarded:', 44, currentY + 8, { lineBreak: false });

  const statusVal = (!isBlank && progress) ? (progress.current_status || '').toLowerCase() : '';
  
  drawFormCheckbox(doc, 190, currentY + 7, statusVal === 't', 9);
  doc.fillColor('#854d0e').fontSize(7).font('Helvetica-Bold').text('[T] Training', 203, currentY + 8, { lineBreak: false });

  drawFormCheckbox(doc, 275, currentY + 7, statusVal === 'm', 9);
  doc.fillColor('#6b21a8').fontSize(7).font('Helvetica-Bold').text('[M] Meets Reqs', 288, currentY + 8, { lineBreak: false });

  drawFormCheckbox(doc, 370, currentY + 7, statusVal === 'c', 9);
  doc.fillColor('#166534').fontSize(7).font('Helvetica-Bold').text('[C] Competent', 383, currentY + 8, { lineBreak: false });

  drawFormCheckbox(doc, 465, currentY + 7, statusVal === 'x', 9);
  doc.fillColor('#1e40af').fontSize(7).font('Helvetica-Bold').text('[X] Trainer', 478, currentY + 8, { lineBreak: false });

  currentY += 32;

  // Assessor Details Box
  doc.rect(36, currentY, 523, 76).fillAndStroke('#ffffff', '#cbd5e1');
  doc.fillColor('#334155').fontSize(7.5).font('Helvetica-Bold');

  const assessorName = (!isBlank && progress && progress.assessor_id && userMap) ? (userMap[progress.assessor_id] || `Assessor #${progress.assessor_id}`) : (isBlank ? '____________________________________' : 'Not Signed Off');
  const signDate = (!isBlank && progress && progress.date_signed_off) ? new Date(progress.date_signed_off).toLocaleDateString('en-GB') : (isBlank ? '____ / ____ / ________' : 'Pending');

  doc.text('Assessor Name:', 44, currentY + 8, { lineBreak: false });
  doc.font('Helvetica').text(assessorName, 125, currentY + 8, { width: 180, lineBreak: false });

  doc.font('Helvetica-Bold').text('Date Assessed:', 320, currentY + 8, { lineBreak: false });
  doc.font('Helvetica').text(signDate, 415, currentY + 8, { width: 135, lineBreak: false });

  doc.font('Helvetica-Bold').text('Assessor Signature:', 44, currentY + 24, { lineBreak: false });
  doc.font('Helvetica').text(isBlank ? '____________________________________' : (progress && progress.date_signed_off ? '[Signed Electronically in Compilot]' : '____________________________________'), 140, currentY + 24, { width: 220, lineBreak: false });

  doc.font('Helvetica-Bold').text('Assessor Feedback / Comments:', 44, currentY + 40, { lineBreak: false });
  const commentText = (!isBlank && progress && progress.signoff_comment) ? progress.signoff_comment : (isBlank ? '____________________________________________________________________________________________________' : 'None recorded');
  doc.font('Helvetica').fillColor('#1e293b').fontSize(7).text(commentText, 44, currentY + 52, { width: 505, height: 20, lineBreak: true });

  // Footer
  doc.rect(36, 785, 523, 0.5).fill('#cbd5e1');
  doc.fillColor('#64748b').fontSize(6.5).font('Helvetica').text(
    'This document is a formal record of clinical training and competency under IR(ME)R 2017. Generated electronically by Compilot.',
    36,
    792,
...
async function generateOverviewMatrixPdfBuffer(targetSection = 'all') {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    for (const sec of sectionsToExport) {
      ...
      for (let batchIdx = 0; batchIdx < totalBatches; batchIdx++) {
        const batchUsers = sectionUsers.slice(batchIdx * MAX_USERS_PER_VIEW, (batchIdx + 1) * MAX_USERS_PER_VIEW);
        const userColWidth = batchUsers.length > 0 ? (remainingWidth / batchUsers.length) : remainingWidth;

        if (!isFirstSection || batchIdx > 0) {
          doc.addPage();
        }
        isFirstSection = false;

        const currentPageIndex = doc.bufferedPageRange().count - 1;
        pageSectionMap[currentPageIndex] = secName;

        const renderPageHeader = () => {
          doc.rect(tableLeft, 12, tableWidth, 24).fill('#1e3a8a');
          doc.fillColor('#ffffff').fontSize(11).font('Helvetica-Bold').text(
            `IR(ME)R Competency Matrix: ${secName} Section`,
            tableLeft + 10,
            18,
            { lineBreak: false }
          );
          ...
          doc.fontSize(8).font('Helvetica').text(
            `Staff Group: ${batchIdx * MAX_USERS_PER_VIEW + 1} - ${Math.min((batchIdx + 1) * MAX_USERS_PER_VIEW, sectionUsers.length)} of ${sectionUsers.length} | Exported: ${dateStr}`,
            tableLeft + tableWidth - 280,
            19,
            { width: 270, align: 'right', lineBreak: false }
          );

          const legendY = 40;
          doc.fillColor('#334155').fontSize(6.5).font('Helvetica-Bold').text('STATUS KEY:', tableLeft + 4, legendY + 4, { lineBreak: false });
          const legendBadges = [
            { code: 'T', bg: '#fef08a', text: '#854d0e', label: 'Training' },
            { code: 'M', bg: '#f3e8ff', text: '#6b21a8', label: 'Meets Reqs' },
            { code: 'A', bg: '#fed7aa', text: '#c2410c', label: 'Assessment' },
            { code: 'C', bg: '#bbf7d0', text: '#166534', label: 'Competent' },
            { code: 'X', bg: '#bfdbfe', text: '#1e40af', label: 'Trainer' }
          ];
          let badgeX = tableLeft + 60;
          legendBadges.forEach(b => {
            doc.rect(badgeX, legendY + 2, 12, 10).fill(b.bg);
            doc.fillColor(b.text).fontSize(6).font('Helvetica-Bold').text(b.code, badgeX, legendY + 4, { width: 12, align: 'center', lineBreak: false });
            doc.fillColor('#475569').fontSize(6).font('Helvetica').text(`= ${b.label}`, badgeX + 15, legendY + 4, { lineBreak: false });
            badgeX += 68;
          });
        };

        const renderTableHeader = (headerY) => {
          doc.rect(tableLeft, headerY, tableWidth, 24).fill('#e2e8f0');
          doc.rect(tableLeft, headerY, tableWidth, 24).stroke('#94a3b8');
        ...
        if (comps.length === 0) {
          ...
          comps.forEach((comp) => {
            ...
            const rowY = currentY;
            const isAlt = currentY % 26 === 0;
            doc.rect(tableLeft, rowY, tableWidth, 13).fill(isAlt ? '#f8fafc' : '#ffffff');
            doc.rect(tableLeft, rowY, tableWidth, 13).stroke('#e2e8f0');

            const isNewCat = comp.category !== lastCategory;
            if (isNewCat) {
              lastCategory = comp.category;
              doc.fillColor('#0f172a').fontSize(6.5).font('Helvetica-Bold').text(comp.category || '', tableLeft + 4, rowY + 3, { width: catColWidth - 8, lineBreak: false, ellipsis: true });
            }

            doc.fillColor('#1e293b').fontSize(6.5).font('Helvetica').text(comp.task_name || '', tableLeft + catColWidth + 4, rowY + 3, { width: taskColWidth - 8, lineBreak: false, ellipsis: true });

            batchUsers.forEach((u, uIdx) => {
              const uX = tableLeft + catColWidth + taskColWidth + (uIdx * userColWidth);
              const p = progressMap[u.id + '_' + comp.id];
              const statusCode = p ? (p.current_status || 't').toUpperCase() : 'T';

              const colorMap = {
                'T': { bg: '#fef9c3', text: '#854d0e' },
                'M': { bg: '#f3e8ff', text: '#6b21a8' },
                'A': { bg: '#fed7aa', text: '#c2410c' },
                'C': { bg: '#bbf7d0', text: '#166534' },
                'X': { bg: '#bfdbfe', text: '#1e40af' }
              };
              const badgeStyle = colorMap[statusCode] || colorMap['T'];

              const bWidth = Math.min(16, userColWidth - 4);
              const bHeight = 10;
              const bX = uX + (userColWidth - bWidth) / 2;
              const bY = rowY + 1.5;

              doc.rect(bX, bY, bWidth, bHeight).fill(badgeStyle.bg);
              doc.fillColor(badgeStyle.text).fontSize(6).font('Helvetica-Bold').text(statusCode, bX, bY + 2, { width: bWidth, align: 'center', lineBreak: false });
            });
...
async function generateOverviewMatrixCSV(targetSection = 'all') {
  const allSections = await getSections();
  let sectionsToExport = [];
  if (targetSection === 'all') {
    sectionsToExport = allSections.filter(s => s.active !== false);
  ...
  for (const sec of sectionsToExport) {
    ...
    const progressMap = {};
    progress.forEach(p => { progressMap[p.user_id + '_' + p.competency_id] = p; });

    csv += `Section: ${secName}\r\n`;
    const headers = ['Category', 'Competency Task'];
    sectionUsers.forEach(u => {
      headers.push(`"${u.full_name} (${u.designation}) Status"`);
      headers.push(`"${u.full_name} (${u.designation}) Date Achieved"`);
    });
    csv += headers.join(',') + '\r\n';

    comps.forEach(comp => {
      const row = [`"${(comp.category || '').replace(/"/g, '""')}"`, `"${(comp.task_name || '').replace(/"/g, '""')}"`];
      sectionUsers.forEach(u => {
        const p = progressMap[u.id + '_' + comp.id];
        const status = p ? (p.current_status || 't').toUpperCase() : 'T';
        const dateAchieved = p && ['c', 'x'].includes(p.current_status) && p.date_signed_off ? p.date_signed_off : '-';
        row.push(`"${status}"`);
        row.push(`"${dateAchieved}"`);
      });
      csv += row.join(',') + '\r\n';
    });
    csv += '\r\n';
...
app.get('/api/admin/reports/competency-templates-zip/:section?', authenticateToken, requireAdmin, async (req, res) => {
  ...
  try {
    ...
    const targetSections = targetSection === 'all' ? activeSections.map(s => s.name) : [targetSection];

    let quizzes = [];
    try {
      quizzes = await query(sharedDb, "SELECT id, title FROM quizzes");
    } catch(e) {}
    const quizzesMap = {};
    quizzes.forEach(q => quizzesMap[q.id] = q);
    ...
    for (const secName of targetSections) {
      ...
      let comps = [];
      try {
        comps = await getCompetenciesWithGroups(dbInstance);
      } catch(e) {}
      if (!comps || comps.length === 0) continue;

      const allCompsMap = {};
      comps.forEach(c => allCompsMap[c.id] = c);
      ...
      for (const comp of comps) {
        templateJobs.push(async () => {
          const pdfBuffer = await new Promise((resolve, reject) => {
            ...
            renderCompetencyAssessmentForm(doc, {
              section: secName,
              comp,
              allCompsMap,
              quizzesMap,
          ...
          const safeCategory = (comp.category || 'General').replace(/[^a-zA-Z0-9_\-\.]/g, '_');
          const safeTask = (comp.task_name || 'Competency').replace(/[^a-zA-Z0-9_\-\.]/g, '_');
          const filename = targetSection === 'all' 
            ? `${secName}/${safeCategory} - ${safeTask}.pdf`
            : `${safeCategory} - ${safeTask}.pdf`;
    ...
    if (templateJobs.length === 0) {
      return res.status(404).json({ error: `No competencies found for section: ${targetSection}` });
    }

    const templateResults = await Promise.all(templateJobs.map(job => job()));
    templateResults.forEach(r => zip.addFile(r.filename, r.pdfBuffer));

    const zipFilename = targetSection === 'all'
      ? `competency_templates_all_sections_${new Date().toISOString().split('T')[0]}.zip`
      : `competency_templates_${targetSection.toLowerCase()}_${new Date().toISOString().split('T')[0]}.zip`;

    res.set('Content-Type', 'application/zip');
    res.set('Content-Disposition', `attachment; filename=${zipFilename}`);
    res.send(zip.toBuffer());
  } catch (error) {
    console.error("Error generating competency templates ZIP:", error);
    res.status(500).json({ error: error.message });
  }
});

app.get('/api/admin/reports/user-competency-forms-zip/:section?', authenticateToken, requireAdmin, async (req, res) => {
  req.setTimeout(600000);
  res.setTimeout(600000);
  const targetSection = req.params.section || req.query.section || 'all';
  try {
    ...
    const allUsers = await query(sharedDb, "SELECT * FROM users WHERE is_active = 1 ORDER BY full_name ASC");
    const userMap = {};
    allUsers.forEach(u => userMap[u.id] = u.full_name || u.username);
    ...
    for (const secName of targetSections) {
      ...
      try {
        comps = await getCompetenciesWithGroups(dbInstance);
        progressList = await query(dbInstance, "SELECT * FROM staff_competency_progress");
      } catch(e) {}
      const allCompsMap = {};
      comps.forEach(c => allCompsMap[c.id] = c);
      sectionData[secName] = { comps, progressList, allCompsMap };
    }

    const zip = new AdmZip();

    const userJobs = allUsers.map(u => async () => {
      let userActiveInSections = [];
      try {
        const act = typeof u.active_in === 'string' ? JSON.parse(u.active_in) : u.active_in;
        userActiveInSections = Array.isArray(act) ? act : [];
      } catch(e) { userActiveInSections = targetSections; }

      const applicableSections = targetSections.filter(s => userActiveInSections.includes(s));
      ...
      const pdfBuffer = await new Promise((resolve, reject) => {
        ...
        applicableSections.forEach((secName) => {
          const sData = sectionData[secName];
          if (!sData || sData.comps.length === 0) return;

          const userProgList = sData.progressList.filter(p => p.user_id === u.id);
          const userProgressMap = {};
          userProgList.forEach(p => userProgressMap[p.competency_id] = p);

          const userApplicableComps = sData.comps.filter(comp => isUserApplicableForComp(u, comp));

          userApplicableComps.forEach((comp) => {
            ...
            renderCompetencyAssessmentForm(doc, {
              section: secName,
              comp,
              allCompsMap: sData.allCompsMap,
              quizzesMap,
              user: u,
              progress: prog,
              userAllProgressMap: userProgressMap,
              userMap
            });
          });
        });
        ...
        if (totalPages === 0) {
          doc.fontSize(10).text('No assigned competencies found for this staff member.');
        }
      ...
      if (totalPages > 0) {
        const safeName = (u.full_name || 'User').replace(/[^a-zA-Z0-9_\-\.]/g, '_');
        const safeUser = (u.username || u.id).replace(/[^a-zA-Z0-9_\-\.]/g, '_');
        const filename = targetSection === 'all'
          ? `${safeName}_${safeUser}_Competency_Records_All_Sections.pdf`
          : `${safeName}_${safeUser}_Competency_Records_${targetSection}.pdf`;
        return { filename, pdfBuffer };
      }
      return null;
    });

    const userResults = await Promise.all(userJobs.map(job => job()));
    let totalUsersAdded = 0;
    userResults.forEach(r => {
      if (r) {
        totalUsersAdded++;
        zip.addFile(r.filename, r.pdfBuffer);
      }
    });

    if (totalUsersAdded === 0) {
      return res.status(404).json({ error: `No active staff found for section: ${targetSection}` });
    }

    const zipFilename = targetSection === 'all'
      ? `user_competency_records_all_sections_${new Date().toISOString().split('T')[0]}.zip`
      : `user_competency_records_${targetSection.toLowerCase()}_${new Date().toISOString().split('T')[0]}.zip`;

    res.set('Content-Type', 'application/zip');
    res.set('Content-Disposition', `attachment; filename=${zipFilename}`);
    res.send(zip.toBuffer());
  } catch (error) {
    console.error("Error generating user competency forms ZIP:", error);
    res.status(500).json({ error: error.message });
  }
});

app.get('/api/admin/reports/master-backup-bundle-zip', authenticateToken, requireAdmin, async (req, res) => {
  req.setTimeout(600000);
  res.setTimeout(600000);
  try {
    const sections = await getSections();
    ...
    // 1. Overview Matrices (PDF & CSV)
    try {
      const matrixPdf = await generateOverviewMatrixPdfBuffer('all');
      masterZip.addFile(`01_Overview_Matrices/competency_matrix_all_sections_${dateStr}.pdf`, matrixPdf);

      const matrixCsv = await generateOverviewMatrixCSV('all');
      masterZip.addFile(`01_Overview_Matrices/competency_matrix_all_sections_${dateStr}.csv`, Buffer.from(matrixCsv, 'utf8'));
    } catch(e) { console.error("Master bundle matrix error:", e.message); }

    // Preload shared users & quizzes & section data
    const allUsers = await query(sharedDb, "SELECT * FROM users WHERE is_active = 1 ORDER BY full_name ASC");
    const userMap = {};
    allUsers.forEach(u => { userMap[u.id] = u.full_name || u.username; });

    let quizzes = [];
    try { quizzes = await query(sharedDb, "SELECT id, title FROM quizzes"); } catch(e) {}
    const quizzesMap = {};
    quizzes.forEach(q => quizzesMap[q.id] = q);
    ...
    for (const sec of activeSections) {
      ...
      const progressList = await query(dbInstance, "SELECT * FROM staff_competency_progress");
      let auditLogs = [];
      try { auditLogs = await query(dbInstance, "SELECT * FROM competency_audit_log ORDER BY timestamp DESC"); } catch (e) {}
      const allCompsMap = {};
      comps.forEach(c => allCompsMap[c.id] = c);
      sectionData[sec.name] = { competencies: comps, comps, progressList, auditLogs, allCompsMap };
    }

    // 2. User Training Records Portfolios (Parallel Generation)
    try {
      const portfolioJobs = allUsers.map(u => async () => {
        const userActiveSections = [];
        try {
          const parsed = typeof u.active_in === 'string' ? JSON.parse(u.active_in) : u.active_in;
          if (Array.isArray(parsed)) userActiveSections.push(...parsed);
        } catch(e) {}
        const applicableSections = activeSections.filter(s => userActiveSections.includes(s.name));
        ...
        const pdfBuffer = await new Promise((resolve, reject) => {
          ...
          doc.rect(36, 36, 523, 40).fill('#1e3a8a');
          doc.fillColor('#ffffff').fontSize(14).font('Helvetica-Bold').text('CLINICAL TRAINING & COMPETENCY PORTFOLIO', 46, 45);
          doc.fontSize(8).font('Helvetica').text(`IR(ME)R 2017 Staff Record | Generated: ${new Date().toLocaleDateString('en-GB')}`, 46, 62);

          doc.rect(36, 84, 523, 46).fillAndStroke('#f8fafc', '#cbd5e1');
          doc.fillColor('#334155').fontSize(8).font('Helvetica-Bold');
          doc.text('Full Name:', 44, 92);
          doc.font('Helvetica').text(u.full_name || 'N/A', 110, 92);
          doc.font('Helvetica-Bold').text('Designation:', 320, 92);
          doc.font('Helvetica').text(u.designation || 'N/A', 390, 92);
          doc.font('Helvetica-Bold').text('Username:', 44, 106);
          doc.font('Helvetica').text(u.username || 'N/A', 110, 106);
          doc.font('Helvetica-Bold').text('Date in Post:', 320, 106);
          doc.font('Helvetica').text(u.date_in_post ? new Date(u.date_in_post).toLocaleDateString('en-GB') : 'N/A', 390, 106);

          let currentY = 140;
          for (const sec of applicableSections) {
            const sData = sectionData[sec.name];
            if (!sData || sData.competencies.length === 0) continue;

            const userApplicableComps = sData.competencies.filter(comp => isUserApplicableForComp(u, comp));
            if (userApplicableComps.length === 0) continue;

            if (currentY > 700) { doc.addPage(); currentY = 40; }
            doc.rect(36, currentY, 523, 18).fill('#0f172a');
            doc.fillColor('#ffffff').fontSize(9).font('Helvetica-Bold').text(`SECTION: ${sec.name.toUpperCase()}`, 44, currentY + 5);
            currentY += 22;

            const userProg = {};
            sData.progressList.filter(p => p.user_id === u.id).forEach(p => { userProg[p.competency_id] = p; });

            userApplicableComps.forEach((comp) => {
              if (currentY > 740) { doc.addPage(); currentY = 40; }
              const p = userProg[comp.id];
              const statusCode = p ? (p.current_status || 't').toUpperCase() : 'T';
              const isCompetent = ['C', 'X'].includes(statusCode);

              doc.rect(36, currentY, 523, 14).fill('#f8fafc').stroke('#e2e8f0');
              doc.fillColor('#1e293b').fontSize(7.5).font('Helvetica-Bold').text(comp.task_name, 44, currentY + 3, { width: 280, ellipsis: true });
              doc.fillColor(isCompetent ? '#166534' : '#854d0e').fontSize(7).font('Helvetica-Bold').text(`Status: [${statusCode}]`, 330, currentY + 3);
              const signDate = p && p.date_signed_off ? new Date(p.date_signed_off).toLocaleDateString('en-GB') : '-';
              doc.fillColor('#475569').fontSize(7).font('Helvetica').text(`Achieved: ${signDate}`, 410, currentY + 3);
              currentY += 16;
            });
            currentY += 10;
          }
        ...
        const safeFilename = `${(u.full_name || 'User').replace(/[^a-zA-Z0-9_\-\.]/g, '_')}_${(u.username || u.id).replace(/[^a-zA-Z0-9_\-\.]/g, '_')}_Training_Record.pdf`;
        return { path: `02_User_Training_Records_Portfolios/${safeFilename}`, buffer: pdfBuffer };
      });

      const portfolioResults = await Promise.all(portfolioJobs.map(j => j()));
      portfolioResults.forEach(r => { if (r) masterZip.addFile(r.path, r.buffer); });
    } catch(e) { console.error("Master bundle portfolios error:", e.message); }

    // 3. Blank Competency Forms Templates (Parallel Generation)
    try {
      const templateJobs = [];
      for (const sec of activeSections) {
        const secName = sec.name;
        const sData = sectionData[secName];
        if (!sData || !sData.comps) continue;

        for (const comp of sData.comps) {
          templateJobs.push(async () => {
            const pdfBuffer = await new Promise((resolve, reject) => {
              const doc = new PDFDocument({ margin: 36, size: 'A4', bufferPages: false });
              const chunks = [];
              ...
              doc.on('end', () => resolve(Buffer.concat(chunks)));
              doc.on('error', reject);
              renderCompetencyAssessmentForm(doc, { section: secName, comp, allCompsMap: sData.allCompsMap, quizzesMap, user: null, progress: null });
              doc.end();
            });

            const safeCategory = (comp.category || 'General').replace(/[^a-zA-Z0-9_\-\.]/g, '_');
            const safeTask = (comp.task_name || 'Competency').replace(/[^a-zA-Z0-9_\-\.]/g, '_');
            return { path: `03_Blank_Competency_Forms_Templates/${secName}/${safeCategory} - ${safeTask}.pdf`, buffer: pdfBuffer };
          });
        }
      }
      const templateResults = await Promise.all(templateJobs.map(j => j()));
      templateResults.forEach(r => { if (r) masterZip.addFile(r.path, r.buffer); });
    } catch(e) { console.error("Master bundle blank templates error:", e.message); }

    // 4. Completed User Competency Forms (Parallel Generation)
    try {
      const completedUserJobs = allUsers.map(u => async () => {
        let userActiveInSections = [];
        try {
          const act = typeof u.active_in === 'string' ? JSON.parse(u.active_in) : u.active_in;
          userActiveInSections = Array.isArray(act) ? act : [];
        } catch(e) { userActiveInSections = activeSections.map(s => s.name); }

        const applicableSections = activeSections.filter(s => userActiveInSections.includes(s.name)).map(s => s.name);
        if (applicableSections.length === 0) return null;
        ...
        const pdfBuffer = await new Promise((resolve, reject) => {
          ...
          applicableSections.forEach((secName) => {
            const sData = sectionData[secName];
            if (!sData || !sData.comps || sData.comps.length === 0) return;
        ...
        if (totalPages > 0) {
          const safeName = (u.full_name || 'User').replace(/[^a-zA-Z0-9_\-\.]/g, '_');
          const safeUser = (u.username || u.id).replace(/[^a-zA-Z0-9_\-\.]/g, '_');
          return { path: `04_Completed_User_Competency_Forms/${safeName}_${safeUser}_Competency_Records.pdf`, buffer: pdfBuffer };
        }
        return null;
      });

      const completedResults = await Promise.all(completedUserJobs.map(j => j()));
      completedResults.forEach(r => { if (r) masterZip.addFile(r.path, r.buffer); });
    } catch(e) { console.error("Master bundle completed user forms error:", e.message); }

    // 5. Database JSON Backups
    try {
      const filesToBackup = ['shared', ...activeSections.map(s => s.name)];
      const backupData = {};
      for (const db of filesToBackup) {
        const dbPath = path.resolve(__dirname, `${db}.db`);
        if (fs.existsSync(dbPath)) {
      ...
      masterZip.addFile(`05_Database_JSON_Backups/all_databases_backup_${dateStr}.json`, Buffer.from(JSON.stringify({ backup: backupData, date: dateStr, note: "Full system backup exported by Compilot Master Bundle" }, null, 2), 'utf8'));
    } catch(e) { console.error("Master bundle database JSON error:", e.message); }

    res.set('Content-Type', 'application/zip');
    res.set('Content-Disposition', `attachment; filename=complete_clinical_backup_bundle_${dateStr}.zip`);
    res.send(masterZip.toBuffer());
  } catch (error) {
    console.error("Error generating master backup bundle ZIP:", error);
    res.status(500).json({ error: error.message });
  }
});
